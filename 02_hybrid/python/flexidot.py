...
Bioinformatics, 2018, doi 10.1093/bioinformatics/bty395

FlexiDot version1.04 (aka lab version 137)
...
def load_modules():
    ...
    global cllct, gridspec, patches, rcParams, mplrc, P, Color, SeqIO, np, ccv, mcolors, rgb2hex, regex
    ...
    from matplotlib.colors import colorConverter as ccv
    ...
    rcParams['font.sans-serif'] = ['Helvetica', 'Verdana', 'Tahoma', ]
...
def usage():
    ...
    "FlexiDot: Highly customizable ambiguity-aware dotplots for visual sequence investigation"
    Bioinformatics, doi: 10.1093/bioinformatics/bty395
    ...
    -k, --wordsize                 Wordsize (kmer length) for dotplot comparison [default = 7]
...
def read_gff_color_config(gff_color_config_file=""):
    ...
    # read custom gff_color_config_file log_file_name
...
def legend_figure(colors, lcs_shading_num, type_nuc=True, unit="%", filetype="png", max_lcs_len=None, min_lcs_len=0, bins=[], alphas=[], gff_legend=False, prefix="", verbose=False):
    """
    ...
    if not gff_legend:
        ...
        # specify intervals, if max_lcs_len provided
        if max_lcs_len != None:
            multi_factor = 100 # one digit
            if max_lcs_len <= 1:
                multi_factor = 1000 # two digits
            # len_interval_size = (max_lcs_len-min_lcs_len) * multi_factor *1. // lcs_shading_num * (1./ multi_factor)
            len_interval_size = (max_lcs_len-min_lcs_len) * 1. / lcs_shading_num
            len_pos = [float("%.2f" % (min_lcs_len))]
            # calculate interval positions
            for idx in range(lcs_shading_num):
                len_pos.append(float("%.2f" % (len_pos[-1] + len_interval_size)))

            if prefix.startswith("custom-matrix") and (0 <= max_lcs_len <= 100 and 0 <= min_lcs_len <= 100):
                unit = "%"
            elif prefix.startswith("custom-matrix"):
                unit = ""

            text = "\n%d Legend intervals from %.2f to %.2f: \n\t%s - number: %d, step: %.2f, unit: %s\n" % (lcs_shading_num+1, min_lcs_len, max_lcs_len, str(len_pos), len(len_pos), len_interval_size, unit)
            logprint(text, start=False, printing=True)
            pos = len_pos
        ...
        if bins != []: # labels provided
            ...
            if max_lcs_len != None:
                if num == int(num) and int(len_interval_size) == len_interval_size:
                    legend_labels_lengths[-1] = "%d %s" % (max_lcs_len, unit)
                else:
                    legend_labels_lengths[-1] = "%.2f %s" % (max_lcs_len, unit)
    ...
    elif max_lcs_len != None: 
        label_text = legend_labels_lengths[:]
        edge_col = "black"
        legend_file_name = "Polydotplot_LCS_Shading_Legend_max%d%s_n%d." % (max_lcs_len, unit, lcs_shading_num) + filetype
    elif bins != []:
        label_text = legend_labels_lengths[:]
        edge_col = "black"
        legend_file_name = "Polydotplot_LCS_Shading_Legend_%d%s_n%d." % (bins[0], unit, lcs_shading_num) + filetype
    else:
        label_text = legend_labels[:]
        edge_col = "black"
        legend_file_name = "Polydotplot_LCS_Shading_Legend_%%len_n%d." % lcs_shading_num + filetype
...
def pairdotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, substitution_count=0, alphabetic_sorting=False, mirror_y_axis=False, title_length=float("Inf"), title_clip_pos="B", max_N_percentage=49, verbose=False, multi=True, ncols=4, nrows=5, x_label_pos_top=True, length_scaling=True, scale_delim_col="red"):
    ...
    lcs_data_file = open("%sPairdotplot_lcs_data_file%s.txt" % (prefix, suffix.replace("_scaled", "").replace("_collage", "")), 'w')
...
def polydotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, substitution_count=0, alphabetic_sorting=False, mirror_y_axis=False, title_length=float("Inf"), title_clip_pos="B", max_N_percentage=49, verbose=False, gff_files=[], gff_color_dict={"others": ("grey", 1, 0)}, x_label_pos_top=True, lcs_shading=True, lcs_shading_ref=0, lcs_shading_interval_len=100, lcs_shading_ori=0, lcs_shading_num=5, spacing=0.04, input_user_matrix_file="", user_matrix_print=True, rotate_labels=False):
    ...
    if lcs_shading:
        if lcs_shading_ref == 1: # percentage of shorter sequence
            ...
            legend_file_name = legend_figure(colors, lcs_shading_num, unit=aa_bp_unit, filetype=filetype, prefix=prefix, max_lcs_len=max_lcs)

    if custom_shading:
        custom_prefix = "custom-matrix-" + prefix
        legend_file_name_custom = legend_figure(colors_2, lcs_shading_num, unit="%", filetype=filetype, prefix=custom_prefix, max_lcs_len=custom_max, min_lcs_len=custom_min)
