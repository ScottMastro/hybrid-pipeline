...
def plot_levels(blockList, trashList, mblockList, contigList, \
    ...
    ymaxChunk = min(19, 0 if chunkRects is None else max(get_ymax(chunkRects)) + 1)
    ymaxBlock =  min(19, 0 if blockRects is None else max(get_ymax(blockRects)) + 1)
    ymaxMblock =  min(19, 0 if mblockRects is None else max(get_ymax(mblockRects)) + 1)
    ymaxContig =  min(19, 0 if contigRects is None else  max(get_ymax(contigRects)) + 1)
    ...
    if annotate:
        if blockRects is not None:
            for block, path in zip(blocks, blockRects.get_paths()):
                ...
                ax1.annotate(label, xy=(x1, y1 + h), clip_on=True, ha='left', va='center',
                             path_effects=[PathEffects.withStroke(linewidth=2.5,foreground="w")])
                    
                label = str(round(block.end(q=False)/1000.0))
                ax1.annotate(label, xy=(x2, y2 - h), clip_on=True, ha='right', va='center',
                             path_effects=[PathEffects.withStroke(linewidth=2.5,foreground="w")])
            ...
            for mblock, path in zip(mblocks, mblockRects.get_paths()):
                ...
                if x2-x1 < space*2:
                    ...
                    for i in np.arange(x1, x2, space):
                        ...
                        if x > x2: continue
                        label = str(mblock.rid)
                        ax2.annotate(label, xy=(x, (y1+y2)/2), clip_on=True, va='center', ha='center',
                                     path_effects=[PathEffects.withStroke(linewidth=2.5,foreground="w")])
...
def draw_rect_r(intervals, edge):
    
    if len(intervals) < 1:
        return None
        
    intervals.sort(key=lambda interval: interval.left(q=False))
    ...
    while not done:
        ...
        for i in range(len(intervals)):
            if(ycoord[i] < 0):
                ...
                if(lastInterval is None or lastInterval.right(q=False) <= intervals[i].left(q=False)):
                    lastInterval = intervals[i]
                    ycoord[i] = y
        if y==20:
            for i in range(len(intervals)):
    ...
    xcoordLeft = [interval.left(q=False)/1000 for interval in intervals]
    xcoordRight = [interval.right(q=False)/1000 for interval in intervals]
    fills = [interval.fill for interval in intervals]
...
def plot_levels_r(blockList, trashList, mblockList, contigList, \
                length, step=None, outputPath=None, annotate=True):
    ...
    chunkRects = draw_rect_r(chunks, "black")
    blockRects = draw_rect_r(blocks, "black")
    mblockRects = draw_rect_r(mblocks, "black")
    contigRects = draw_rect_r(contig, "black")
    ...
    if annotate:
        if blockRects is not None:
            for block, path in zip(blocks, blockRects.get_paths()):
                ...
                label = str(round(block.start(q=True)/1000.0))
                ax1.annotate(label, xy=(x1, y1), clip_on=True, ha='center',
                             path_effects=[PathEffects.withStroke(linewidth=3,foreground="w")])
                    
                label = str(round(block.end(q=True)/1000.0))
