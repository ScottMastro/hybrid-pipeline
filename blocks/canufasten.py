    ...
    def add_seq(startFork, endFork):
        ...
        if invert:
            ...
            if(startFork.before_strand() != endFork.after_strand()):
                e = len(str(seqData[str(endFork.after_id())])) - endFork.after_pos()
...
def fasten_megablock(megablock, seqData, param, verbose=False):
    rid = megablock.rid
    qid = megablock.qid
...
def fasten_contig(contig, seqData, lengthData, param):
    ...
    for megablock in contig.mblocks:
        path = fasten_megablock(megablock, seqData, param, verbose=False)
        cleanPath = clean_path(path, lengthData, verbose=True)

        (sequence, source) = path_to_sequence(cleanPath, seqData)
        (sequenceInvert, sourceInvert) = path_to_sequence(cleanPath, seqData, invert=True)
        
        f = open("canu_corrected.fasta", "w+")
        g = open("supernova_uncorrected.fasta", "w+")
