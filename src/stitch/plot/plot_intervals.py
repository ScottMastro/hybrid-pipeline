...
def plot_levels(intervalList, length, q=True, outputPath=None, step=500):
    """Creates a visualization of the Interval objects that make up a Contig.
    
    intervalList is a list containing lists of Interval objects to plot:
        [[chunks], [blocks], [megablocks], [contig]]

    length is the number of base pairs in the contig being plotted.
    q is True if plotting a query contig, False if reference contig.
    outputPath is the path (with filename) to output resulting plot.
    step is the # of bp (in thousands) to split the full plot up into.
    
    ex. if length is 2,500,000 and step is 500 then 5 plots will be produced.
    """
