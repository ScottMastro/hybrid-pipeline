...
class Interval:
    ...
    def __init__(self, qid="?", rid="?", cigar=None, subclass=None):
        self.rid=str(rid)
        self.qid=str(qid)
        self.cigar=cigar
...
class Chunk(Interval):
    ...
    def coverage(self): return (self.alen*self.pcid)/100.0
    def coverage_between(self, pos1, pos2, q=True):
...
class Contig:
    ...
    def add(self, block): self.components.append(block)  
    def sort(self, q=True): self.components.sort(key=lambda c: c.start(q))
    ...
    def percent_identity(self):
        '''Average percent identity of all components'''
        return sum([x.percent_identity() for x in mblockself.components])/len(self.components)
    def percent_identities(self): 
        '''Gets the percent identity of all components, returns as list'''
        return [x.percent_identity() for x in self.components]
...
class Block:
    ...
    def __init__(self, qregion, rregion, strand, percentIdentity, alignLen, cigar=None):
        self.query=qregion
        self.ref=rregion
        
        self.qid = qregion.chrom
        self.rid = rregion.chrom

        self.pcid = percentIdentity
        self.alen = alignLen
        self.strand = strand

        self.cigar=cigar
        
    def start(self, q=True): return self.query.start if q else self.ref.start
    def end(self, q=True): return self.query.end if q else self.ref.end
    ...
    def get_dir(self, q=True, string=False):
        if self.strand == 1: return "+" if string else 1
        if self.strand == -1:  return "-" if string else -1
    ...
    def __eq__(self, other): 
        ...
        if not other.qregion == self.qregion: return False
        if not other.rregion == self.rregion: return False

        return True
            
    def __repr__(self):
        return self.get_dir(string=True) + "\t" + str(self.query) + "\t" + str(self.ref)
    def __str__(self):
        return self.__repr__() 
