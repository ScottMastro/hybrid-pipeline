...
def read_fasta(fasta, description=False):
    ...
    seq = dict(zip([str(r.id) for r in records], [str(r.seq) for r in records]))
    
    if description:
        desc = dict(zip([str(r.id) for r in records], [str(r.description) for r in records]))
        return (seq,desc) 
    return seq
