import polish_region_implementation as polisher
import polish_region_log as analyzer
...
def get_region(fork1, fork2, lengthData, q=True):
    ...
    return rgn.SimpleRegion(tid, min(p[0], p[1]), max(p[0], p[1]))
    ...
    if len(blockPath) > 1:
        ...
        for i,fork in enumerate(blockPath[:-1]):
            if fork.after_id() == blockPath[i+1].before_id():
                ...
                region = rgn.SimpleRegion(fork.after_id(), fork.after_pos(), blockPath[i+1].before_pos())
...
def polish_interMblock(blockPathBefore, blockPathAfter, seqData, lengthData, param):
    ...
    ridL = startFork.rid
    rRegionLeft = rgn.SimpleRegion(ridL, max(0, posL-1000), min(lengthData[ridL], posL+1000))
   
    posR = endFork.get_pos_norm(lengthData, q=False)
    ridR = endFork.rid
    rRegionRight = rgn.SimpleRegion(ridR, max(0, posR-1000), min(lengthData[ridR], posR+1000))
