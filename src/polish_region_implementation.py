...
def clean_directory(outdir):
    ...
    #fastas
    helper.delete_file(outdir + "unpolished_plusflank.fasta")
    helper.delete_file(outdir + "consensus.fasta.fxi")
    #helper.delete_file(outdir + "ref_polished.fasta")

    helper.delete_file(outdir + "A_polish.consensus.fasta")
    helper.delete_file(outdir + "B_polish.consensus.fasta")
    
    #vcfs
    helper.delete_file(outdir + "phased.longshot.vcf")
    helper.delete_file(outdir + "high_confidence_hets.vcf")
    helper.delete_file(outdir + "high_confidence.whatshap.vcf.gz.tbi")

    helper.delete_file(outdir + "A_polish.consensus.vcf")
    helper.delete_file(outdir + "B_polish.consensus.vcf")


    #bams
    helper.delete_file(outdir + "hapA_backaligned.pbmm2.bam")
    helper.delete_file(outdir + "hapB_backaligned.pbmm2.bam")
    
    helper.delete_file(outdir + "ref_aligned_consensus.pbmm2.bam")
    #helper.delete_file(outdir + "ref_aligned_consensus.pbmm2.haplotag.bam")

    helper.delete_file(outdir + "q_to_consensus.paf")

    #directories
    helper.delete_file(outdir + "sdf/", deleteDirTree=True)
    helper.delete_file(outdir + "refdata-consensus/", deleteDirTree=True)
    helper.delete_file(outdir + "vcfeval_out/", deleteDirTree=True)
...
def polish_across_gap(gapFa, refBam, seqData, outdir, param, outName="ref_polished"):
    ...
    helper.delete_file(re.sub(".fasta", ".vcf", polishedFa))       
...
def call_variants_query(consensusFa, queryBam, outdir, param):
    ...
    helper.delete_file(normalizedLongranger)
...
def call_variants_ref(consensusFa, refBam, outdir, param):
    ...
    helper.delete_file(vcfA_GT) ; helper.delete_file(vcfB_GT)
    helper.delete_file(normalizedA) ; helper.delete_file(normalizedB)
    helper.delete_file(tempVCF)
...
def align_to_consensus_query(consensusFa, queryReads, qRegion, seqData, outdir, param):
    ...
    helper.delete_file(longrangerVCF)
...
def align_to_consensus_ref(consensusFa, refReads, rRegion, seqData, outdir, param):
    ...
    consensusRegion = rgn.SimpleRegion(seqName, 0, len(sequenceDict[seqName])-1)
    ...
    helper.delete_file(consensusFlankBam)
    helper.delete_file(emptyBam)
    helper.delete_file(bamHeader)       
...
def polish_query_reads(refPolishedFa, queryReads, qRegion, seqData, outdir, param):
    ...
    helper.move_file(polishedFa, consensusFa)
    tools.samtools_faidx(consensusFa)
    
    for ext in [".sa", ".pac", ".bwt", ".ann", ".amb", ".ann"]:
        helper.delete_file(refPolishedFa + ext)
        
    helper.delete_file(alignedBam)
    helper.delete_file(trimmedr1)
...
def polish_ref_reads(hybridFa, refReads, rRegion, seqData, outdir, param, outName="ref_polished"):
    ...
    polishRegion = rgn.SimpleRegion(seqName, 0, len(sequenceDict[seqName])-1)
    ...
    targetRegion = rgn.SimpleRegion(seqName, 0, len(sequenceDict[seqName])-1)
    ...
    helper.delete_file(polishAlignBam)
    helper.delete_file(emptyBam)
    helper.delete_file(bamHeader)      

    helper.delete_file(flankedFa)
    helper.delete_file(polishedFa)
    helper.delete_file(alignedBam)
    helper.delete_file(re.sub(".fasta", ".vcf", polishedFa))       
...
def fetch_query_reads(bamFile, qRegion, outdir):
    helper.delete_file(outdir + "10x_reads", deleteDirTree=True)
