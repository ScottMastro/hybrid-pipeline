...
'''
 A Path object may contain forks in an arbitrary order with respect to an 
 assembly's coordinate system (ie. base pair positions). To account
 for this, CriticalForks can be created to store the extreme positions
 (ie. min and max) for a given contig in a given path.
'''
...
class CriticalForks:
    ...
    def dist(self, other):        
        return max(0, other.minPos - self.maxPos)
