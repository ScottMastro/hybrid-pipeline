...
Modified by Scott Mastromatteo for Python 3 compatibility
"""
...
def time_track(starting_time, show=True):
    ...
    if show:
        text = "\n\t %s seconds\n" % str(delta)
        logprint(text, start=False, printing=verbose)
    return now
...
def create_color_list(number, color_map=None, logging=False, max_grey="#b0b0b0"):
    """
    ...
    try:
        ...
        steps = int((len(cmaplist)-1)/(number))
    ...
    if logging: logprint(text, start=False, printing=verbose)
...
def read_seq(input_fasta, verbose=False):
    ...
    if type(input_fasta) == list:
        ...
        if len(input_fasta) > 1:
            if verbose:
                print("concatenating fastas...", end=' ')
...
def read_gff_color_config(gff_color_config_file=""):
    ...
    if not "others" in list(gff_feat_colors.keys()):
    ...
    if verbose:
        ...
        logprint (text, printing=verbose) 
    ...
    for input_gff in input_gff_files:
        ...
        for line in in_file:
            if not line.startswith('#') and line.strip() != "":
                ...
                if data[6] == "-":
                    feat_type += "_rev"
                if not feat_type.lower() in list(color_dict.keys()):
                ...
                if not data[0] in list(feat_dict.keys()):
    ...
    if not gff_legend:
        ...
        if bins != []: # labels provided
            ...
            if len(bins) == len(colors) - 1:
                legend_labels_lengths.append("[%d %s, %s]" % (max(bins), unit, "\u221E")) # infinite
        ...
        for kmer in match_list:
            for i in pos_dict1[kmer]:
                for j in pos_dict2[kmer]:
                    ...
                    if not diag in list(diag_dict.keys()):
...
def polydotplot(input_fasta, wordsize, prefix=None, plot_size=10, label_size=10, filetype='png', type_nuc=True, convert_wobbles=False, substitution_count=0, alphabetic_sorting=False, mirror_y_axis=False, title_length=float("Inf"), title_clip_pos="B", max_N_percentage=49, verbose=False, gff_files=[], gff_color_dict={"others": ("grey", 1, 0)}, x_label_pos_top=True, lcs_shading=True, lcs_shading_ref=0, lcs_shading_interval_len=100, lcs_shading_ori=0, lcs_shading_num=5, spacing=0.04, input_user_matrix_file="", user_matrix_print=True, rotate_labels=False):
    ...
    if lcs_shading or custom_shading:
        ...
        #matrix colour map
        colors_2 = create_color_list(lcs_shading_num+1, color_map="None", logging=True)
    ...
    print("\nCalculating shared regions and lengths of longest_common_substring...", end=' ') 
    ...
    for idx in range(len(sequences)):
        if verbose:
            print("\n%d\t%s vs." % ((counter+1), sequences[idx]), end=' ')
        ...
        for jdx in range(idx, len(sequences)):
            ...
            if custom_shading:
                ...
                if type(custom_value) == int or type(custom_value) == float: 
                    diff = custom_max-custom_min
                    color_idx0 = int((diff)*lcs_shading_num // (1 if diff == 0 else diff))
    ...
    if verbose:
        try:
            logprint(lcs_text, start=False, printing=verbose)
        except:
            pass
