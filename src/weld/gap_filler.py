...
def fill_NNNs_side(qSeq, rSeq, block, qpos, side, extension, param):
    ...
    if abs(qp - qpos) > 20000:     
        print(extension)
        logger.Logger().out("No corresponding sequence in reference was found on " + side + " side. Skipping N filling.", 1)
        return None
...
def fill_NNNs(qSeq, rSeq, block, param):
    ...
    logger.Logger().out("Finding Ns from position " + str(minPos) + " to " + str(maxPos) + ".", 2)
        
    for qstart, qend in find_NNNs(qSeq, minPos, maxPos):

        logger.Logger().out("Ns found between position " + str(qstart) + " to " + str(qend) + ".", 2)        
        logger.Logger().out("Attempting to fill Ns...", 2)
        ...
        while True:
            ...
            if startFork is None or endFork is None:
                if retries > 0:
                    ...
                    logger.Logger().out("N filling unsuccessful. Giving up.", 2)
            ...
            logger.Logger().out("N filling successful.", 2)
    ...
    logger.Logger().out("N filling complete.", 2)
    return pathN