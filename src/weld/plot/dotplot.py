...
def draw_dotplot(nameDict, seqs, hybridSourceList, outdir, prefix=None, wordSize=None):

    '''
    Draws a dotplot. Sequence names are stored in nameDict, base pair characters
    are stored as a list in seqs. hybridSourceList reports if bp in hybrid is from
    query or reference.
    QUERY, REF, HYBRID, HREF are then plotted using flexidot.
    '''
...
def plot_gap(startFork, endFork, seqData, lengthData, buffer, \
                     outdir, prefix=None, wordSize=None):
    '''
    Takes a pair of Forks. Query and reference sequence along this range is 
    extracted from seqData. Hybrid sequence is also generated from sequence segments.
    The corresponding Human Reference Genome sequence is also found using BLAST.
    All four sequences are plotted in a dotplot.
    '''
    nameDict = dict()
    ...
    print("BLASTing against human reference...")
    seq, chrom, start, end = search_reference(seqs[0])
    if seq is None: return
    nameDict[HREF] = chrom + ":" + str(start) + "-" + str(end)
    seqs.append(seq)

    draw_dotplot(nameDict, seqs, hybridSourceList, outdir, prefix=prefix, wordSize=wordSize)

def plot_path(path, seqData, lengthData, outdir, prefix=None, wordSize=None, buffer=0):
    '''
    Takes in a Path. Min and max positions along path are extracted.
    Query and reference sequence along this range is taken from seqData.
    Hybrid sequence is also generated by following the Forks along the Path.
    The corresponding Human Reference Genome sequence is also found using BLAST.
    All four sequences are plotted in a dotplot.
    '''
    nameDict = dict()
...
def gaps_dotplot(blockPaths, seqData, lengthData, outdir):
    '''
    Expects a list of Paths formed from Blocks. Creates a dotplot for each pair of Forks
    along Path, corresponding to the Fork range + some buffer. Also creates
    a dotplot for region between Paths in the blockPaths list.
    
    In other words, a dotplot is created for regions filled within a Block and
    regions between Blocks.
    '''
    prevPath = None
    for pid,path in enumerate(blockPaths):
        ...
        for fid,fork in enumerate(path):
            ...
            if prevFork is not None and prevFork.is_switch_reference():
                plot_gap(prevFork, fork, seqData, lengthData, 1000, outdir, prefix=prefix)
        ...
        if prevPath is not None:
            plot_gap(prevPath[-1], path[0], seqData, lengthData,2500, outdir, prefix=prefix)

    prevPath = path
            
def paths_dotplot(blockPaths, seqData, lengthData, outdir, gaps=True, paths=True):
    '''
    Expects a list of Paths formed from Blocks. Creates a dotplot for entire Path.
    '''
    for pid,path in enumerate(blockPaths):
...
def megapath_dotplot(megaPath, seqData, lengthData, outdir):
    '''
    Expects a list of Paths formed from Megablocks. Creates a dotplot for entire Path.
    '''
